...
const Editor: NextPage = () => {
  ...
  */
  ...
  useEffect(() => {
    if (width === grid[0]!.length) return;
    let newGrid = grid.slice();
    const isExpanding = width > grid[0]!.length;

    if (isExpanding) {
      newGrid = newGrid.map((row) => [...row, null]);
      newGrid.push(new Array(width).fill(null));
    } else {
      newGrid = newGrid.slice(0, width);
      newGrid = newGrid.map((row) => row.slice(0, width));
    }
  ...
  /*
    Components
    TODO: hide these in a separate file
  */

  const SavingModal = () => (
    <div className="absolute h-full w-full grid place-items-center z-40 bg-black/80">
      <form
        className="absolute z-10 p-4 flex flex-col items-center bg-gray-200 border-2 border-black"
      ...
      >
        ...
        <br />
        <input
          className="block p-2 border-2 border-black text-center text-xl"
          type="text"
          placeholder="Name"
          required
          value={formData.name}
        ...
        <div className="block p-2 text-center text-xl mb-4">
          <input
            ...
            onChange={(event) =>
              setFormData({ ...formData, unlisted: event.target.checked })
            }
            className="mr-2"
          />
          Private
        ...
        <button
          className="p-2 border-2 border-black text-xl bg-white"
          type="submit"
        >
          Create
        </button>
  ...
  const ResultModal = () => (
    <div className="absolute h-full w-full grid place-items-center z-50 bg-black/80">
      <div className="absolute z-10 py-8 px-4 flex flex-col items-center bg-gray-200 border-2 border-black">
        ...
        <input
          className="my-2"
          type="text"
          value={`${location.protocol + "//" + location.host}/puzzle/${
            data?.id
          }`}
        />
        <Link href={`/puzzle/${data?.id}`}>
          <a className="text-xl text-blue-400 my-2">Go to level</a>
        </Link>
      </div>
    </div>
  );
    ...
    <div className="flex flex-row justify-between items-center">
      <button className="relative w-8 h-8 lg:w-12 lg:h-12 border-2 border-black bg-gray-200">
        <div
          ...
          style={{
            backgroundColor: selectedColor === null ? "" : selectedColor,
          }}
      ...
      <div className="flex flex-row items-center justify-center mx-8 lg:mx-16">
        <button
          className="border-2 border-black w-8 h-8 lg:w-12 lg:h-12 bg-gray-300 grid place-items-center disabled:opacity-50"
      ...
      <button
        className="text-xl font-semibold border-2 border-black w-8 h-8 lg:w-12 lg:h-12 bg-gray-300"
        onClick={clearGrid}
      >
        {" "}
        ⟳
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Create custom nonogram puzzles to play and share!"
        />
      </Head>
      {data && isSaving && !isLoading && ResultModal()}
      {nameModalOpen && SavingModal()}
      <div className="h-full min-h-fit flex flex-col items-center justify-evenly py-4">
        ...
        <ToolBar />
        <table
          className="relative my-8 hover:cursor-pointer"
            ...
            {grid.map((row, i) => (
              <tr key={`row-${i}`}>
                {row.map((cell, j) => (
                  <td
                    key={`cell-${i}-${j}`}
                    className={`border-[1px] border-gray-600 relative`}
                    style={{
                      backgroundColor:
                        cell !== null ? cell : "rgba(25, 25, 25, 1)",
                      width: cellSize,
                      height: cellSize,
                    }}
                    onPointerOver={(
        ...
        <button
          className="w-24 text-center text-xl font-semibold border-2 border-black py-2 bg-gray-300"
